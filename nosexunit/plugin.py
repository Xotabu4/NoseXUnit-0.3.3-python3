...
class NoseXUnit(Plugin, object):
    ...
    def initialize(self):
        ...
        if self.audit:
            ...
            self.audit_packages = [ package for package in list(self.packages.keys()) if package.find('.') == -1 and self.enable(package) ]
            # Check at least one
        ...
        if self.cover:
            ...
            self.cover_packages = [ package for package in list(self.packages.keys()) if self.enable(package) ]
            # Check at least one
    ...
    def report(self, stream):
        ...
        if self.cover:
            ...
            entries = [ package for entry, package in list(sys.modules.items()) if self.consider(entry, package) ]
            # Stop coverage
