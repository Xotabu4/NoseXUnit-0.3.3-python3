...
class Sources(dict):
    ...
    def listing(self):
        ...
        def _internal(source, slist):
            ...
            for child in list(source.keys()):
                # Recursive process of each child
                _internal(source[child], slist)
        # Result list
        slist = []
        # Start listing
        for source in list(self.keys()): _internal(self[source], slist)
...
def generate(listing):
    ...
    for package in list(listing.keys()):
        # Create the class
        functions = [] 
        # Go threw the tests
...
def check_pylintrc(path=None):
    ...
    if path:
        ...
        if os.path.isfile(path):
            ...
            for section in list(forbid.keys()):
...
def outputs():
    ...
    # Add default outputs
    out.extend(list(pylint.lint.REPORTER_OPT_MAP.keys()))
...
def available():
    ...
    # Unable to get it
    except BaseException as e: return False, str(e)    # pylint: disable-msg=E0601
    # OK, it's available
    return True, None
