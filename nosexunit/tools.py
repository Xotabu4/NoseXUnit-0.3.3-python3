    ...
    for folder, folders, files in os.walk(root):
        ...
        for fld in [ os.path.join(folder, fld) for fld in folders ]:
            ...
            if os.path.exists(os.path.join(fld, nconst.INIT)):
                ...
                if entry in pkgs: logger.warn('package %s already exists in following folder tree: %s' % (entry, root))
                # Else add it
                else: pkgs[entry] = fld
        ...
        for path in [ os.path.join(folder, fn) for fn in files ]:
            ...
            if split(path)[1] == 'py' and os.path.basename(path) != nconst.INIT:
                ...
                entry = package(path)
                # Check if already exists
                if entry in pkgs: logger.warn('module %s already exists in following folder tree: %s' % (entry, root))
                # Else add it
...
def expand(environ):
    ...
    for entry in list(environ.keys()):
